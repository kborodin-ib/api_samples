        ...
        while True:
            ...
            if 'topic' in jsonData.keys():
                ...
                #    response = f"{jsonData['topic']} -->{json.dumps(jsonData, indent=4)}"
                    if "status" or "order_ccp_status" in jsonData["args"][0].keys():
                        print(jsonData["args"][0].keys())
                        print(f'Order status update: {jsonData["args"][0]}')
...
def liveMarketData():
    msg = create_SMD_req('12087792', '6509,7308,7309,7310,7311')
    print(type(msg))
    messages = [msg]
    asyncio.get_event_loop().run_until_complete(sendMessages(messages))

def LiveOrderUpdatesSOR():
    unsubscribeLiveUpdates = ["uor+{}"]
    msg = create_SOR_req()
    messages = [unsubscribeLiveUpdates, msg]
    asyncio.get_event_loop().run_until_complete(sendMessages(messages))
